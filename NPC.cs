...
public void HandleUserTextInput(string userMessage)
{

    if (!string.IsNullOrEmpty(userMessage))
    {
        Debug.Log("User text input: " + userMessage);

      
        string response = "Sorry, I didn't catch that.";

        if (userMessage.Contains("hello"))
        {
            response = "Hi, welcome to the VXLab!";
            animator.SetTrigger("WaveTrigger");
        }
        else if (userMessage.Contains("school"))
        {
            response = "The school of computing technologies at RMIT focuses on AI, cyber security, and more.";
        }
        else if (userMessage.Contains("robot"))
        {
            response = "We have several robots like Rosie and Tiago!";
        }
    ...
    else
    {
        ...
        StartCoroutine(UpdateOutputText("Please type something!"));
    }
}
